...
<script type="text/javascript">
    ...
    app.controller('RegisterCTRL' , function ($scope , $http , $q , $timeout) {
        ...
        $scope.registration = function () {
            ...
            var q = $q.defer() ;
            $http.post('{{route('register.submit')}}' , $scope.register.data).then(
                function (resp) {
                ...
                if($scope.register.response.success !== undefined && $scope.register.response.success === true){
                    ...
                    $timeout(
                        window.location.href = '{{route('dashboard')}}' , 2000
                    )
    ...
    app.controller('ProductCTRL' , function ($scope , $http , $q) {
            ...
            $scope.getSearchResult = function () {
                ...
                $http.post('{{route('products.search')}}', $scope.products.terms).then(
                    function (resp) {
                        q.resolve(resp);
                        console.log(resp.data);
                        ...
                        $scope.prev_page_url = resp.prev_page_url;
                        $scope.next_page_url = resp.next_page_url;
                    }, function (reason) {
            ...
            $scope.getpaginationResult = function (url) {
                if(url != null){
                $scope.products.progress = true;

                var q = $q.defer();

                $http.post(url, $scope.products.terms).then(
                    function (resp) {
                        q.resolve(resp);
                        console.log(resp.data);
                        $scope.products.data = resp.data.data;
...
<!-- <script type="text/javascript" src="{{asset('js/plugins/robonegotiator-sales-plugin.js?ver=2.0')}}"></script> -->
<!-- <script type="text/javascript" src="{{asset('js/plugins/robonegotiator.plugin.js?ver=2.0')}}"></script> -->
<!-- <script type="text/javascript" src="{{asset('js/plugins/robonegotiator.auto.js?ver=2.0')}}"></script> -->
<script type="text/javascript" src="{{asset('js/plugins/new-js/roboconfig.js')}}"></script>




@if(Request::route()->getName() === 'buyer.product.details')
