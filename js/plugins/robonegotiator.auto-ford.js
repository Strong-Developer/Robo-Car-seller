...
function closeChatBotRobo() {
    if(sendBuyerOfferStatus === 1) {
        if(document.getElementById('chatbotrobo')) {
            var htmlContent = escape(document.getElementById('chatbotrobo').innerHTML);
            var trnscriptURL = API_ENDPOINT+"/addTranscription";
            var transSCriptPostData = 'seller_email='+ROBO_SELLER_EMAIL+'&upc_product_code='+product_code+'&transcription='+htmlContent;
            var xmlhttp = new XMLHttpRequest();
            var url = trnscriptURL+transSCriptPostData;
            xmlhttp.onreadystatechange = function () {
                if (this.readyState === 4 && this.status === 200) {
                    var data = JSON.parse(this.responseText);
            ...
            xmlhttp.open("POST", trnscriptURL, true);
            xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            xmlhttp.send(transSCriptPostData);
        }
    }
    if(closeChat === 1) {
