...
couponSection = 0;
fyiSection = 0;
IsPurchasingAndFinanceSection = 0;
...
if(typeof ADD_TO_CART_BTN_IDENTIFIER_TYPE !== "undefined"){
    if (typeof ADD_TO_CART_BTN_IDENTIFIER_NAME !== "undefined") {
        ...
        newButton.setAttribute('class','btn btn-success mr-2 font-weight-semibold btn-v-4 btn-h-2 btn-fs-2');
...
function confirmFinanceMonthOptions() {
    ...
    if(buyerFinanceOptionsValueMonth != ''){
        ...
        var chooseMonthMiletMessage = "Please select your credit score/ history level (* This will help us to offer you appropriate interest rate): <div class='robo-custom-control custom-radio'>"+optionFinanceCreditScoreHtml+"</div><div align='right'><button type='button' class=' robo-btn robo-btn-primary float-right' id='confirm_finance_credit_score_options' onclick='confirmFinanceCreditScoreOptions(event)'>Proceed <i class='fa fa-arrow-right' aria-hidden='true'></i></button></div>";
        textClass = 'text-danger';
        let theEnvelope = {
...
function goToFinanceTradeIn() {
    ...
    if(financeUpfrontPayment != '') {
        ...
        var leaseTadeInMessage = "Are you planning to trade in a car?<div class='robo-custom-control custom-radio'><input id='finance_tradein_option2_yes' type='radio' class='inline-radio' name='finance_tradein_option2_select' value='yes'><label for='finance_tradein_option2_yes'><span></span>Yes</label><input id='finance_tradein_option2_no' type='radio' class='inline-radio' name='finance_tradein_option2_select' value='no'><label for='finance_tradein_option2_no'><span></span>No</label></div><div align='right'><button type='button' class=' robo-btn robo-btn-primary float-right' id='finance_mssg_confirm_lease_tradein' onclick='confirmFinanceTradeIn(event)'>Proceed <i class='fa fa-arrow-right' aria-hidden='true'></i></button></span></div>";
        textClass = 'text-danger';
        let theEnvelope = {
            msg: leaseTadeInMessage,
            sender: false
        ...
        xmlHttpSellerFinanceUpdate.onreadystatechange = function () {
            if (this.readyState === 4 && this.status === 200){
                ...
                if (data.success) {
                    ...
                    var askForCommentMssg ="<div class='robo-custom-control custom-radio'><textarea class='form-control oracle-search' id='askForFinanceComment' placeholder='Please enter message.' style='color:#fff;' name='option1_buyer_message'></textarea></div><div align='right'><button type='button' class=' robo-btn robo-btn-primary float-right' id='finance_ask_for_comment' onclick='checkDealAgainFunctionFinance(event)'>Proceed <i class='fa fa-arrow-right' aria-hidden='true'></i></button></span></div>";
                    textClass = 'text-danger';
                    final_message = {
        ...
        xmlhttp.onreadystatechange = function () {
            ...
            if (this.readyState === 4 && this.status === 200) {
                ...
                if (data.closed_deal === undefined) {
                    ...
                    var offerMessage = '<span class="text-danger">Congratulations!! Our dealer and you have agreed on: <br/><br/>Product/Vehicle: <b style="color:red;">'+ROBO_UPC_CODE+'</b><br/>Mutually agreed sale price: <b style="color:red;">'+roboCurrency+' '+closedDealFinalAmount;
                    if (preferences !== undefined) {
                        //_autoSend(final_message);
                        if(parseInt(data.closed_deal[0].total_discounted_amount) > 0) {
                            ...
                            offerMessage += ' (Includes seller paid rebate/discount: '+roboCurrency+' '+discountAmount+' )</b><br/>';
      ...
      stripe.createToken(card).then(function(result) {
        if (result.error) {
          ...
          $.ajax({
                ...
                method:"POST",
                data:{amount:amount, secret_key:secret_key, currency: roboCurrency, token:result.token.id, description : 'Deposit Against Request Id #'+buyer_offer_request_id},
                dataType:"JSON",
