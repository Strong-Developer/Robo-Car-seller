...
function option2() {
    let theEnvelop = {
        msg: "I want to negotiate and close a deal.",
        sender: false
    };
    _customerReply(theEnvelop);
    var option2Message = "Great! Just to confirm, you want to negotiate for the following product:   <br/> "+product_code+"<div class='robo-custom-control custom-radio'><input id='agree_option2_yes' type='radio' class='inline-radio' name='agree_option2_select' value='yes'><label for='agree_option2_yes'><span>&nbsp;</span>Yes</label>&nbsp;&nbsp;&nbsp;<input id='agree_option2_no' type='radio' class='inline-radio' name='agree_option2_select' value='no'><label for='agree_option2_no'><span>&nbsp;</span>No</label></div><div align='right'><button type='button' class='dha-find-seller-button' id='check_option2_btn' onclick='confirmProduct(event)'>Submit</button><span id='agree_error' style='color:red;'></span></div>";
    setTimeout(function () {
        let theEnvelope = {
        msg: option2Message,
        sender: false
...
{/*  deleted from submit<i class='fa fa-arrow-right' aria-hidden='true'></i> */}
    ...
    xmlhttp.onreadystatechange = function () {
        if (this.readyState === 4 && this.status === 200) {
            ...
            if (data.success !== undefined && data.success === true) {
                var option1Message = "Thanks! We will contact the seller with your message and will get back to you as soon as possible.";
                let theEnvelope = {
                    msg: option1Message,
                    sender: false
                };
...
function option3() {
    let theEnvelop = {
        msg: " I want to check for a discount or rebate.",
        sender: false
    };
    _customerReply(theEnvelop);
    
    chatMsgs[0][1][3] = ['Great! I can help with that. Currently, we have the following discounts:'];
    setTimeout(function () {
        let theEnvelopeusr = {
            msg: chatMsgs[0][1][3],
            sender: false
...
{
	chatMsgs[0][1][31] = ["Would you like to negotiate for a deal? <br/> <div class='robo-custom-control custom-radio'><input id='agree_option3_yes' type='radio' class='inline-radio' name='agree_option2_select' value='yes'><label for='agree_option3_yes'><span>&nbsp;</span>Yes</label>&nbsp;&nbsp;&nbsp;<input id='agree_option3_no' type='radio' class='inline-radio' name='agree_option3_select' value='no'><label for='agree_option2_no'><span>&nbsp;</span>No</label></div><div align='right'><button type='button' class='dha-find-seller-button' id='check_option3_btn' onclick='confirmProduct(event)'>Submit</button><span id='agree_error' style='color:red;'></span></div>"];
	 let theEnvelope = {
			msg: chatMsgs[0][1][31],
			sender: "me"
		};
...
function initChatMsgs(){
    curStep = 1;
    chatMsgs[0]=[];
    chatMsgs[0][1] = ['<div class="inner"><button type="button" id="closebotbtn"  onclick="option1()" class="dha-choose-button">I have a question or comment.</button></div><div class="inner"><button type="button" id="closebotbtn"  onclick="option2()"  class="dha-choose-button">I want to negotiate and close a deal.</button> </div> <div class="inner"><button type="button" id="closebotbtn"  onclick="option3()"  class="dha-choose-button"> I want to check for a discount or rebate.</button></div>'];
    chatMsgs[1]=[];
    chatMsgs[1][0] = ['Hi! Welcome to RoboNegotiator.<br> I am your sales assistant. I can help with anything you need.<br> What do you need help with today?'];
    ...
    chatMsgs[1][1] = ["Let\'s get started then. Please enter your <b>counteroffer</b> below. <div id='counterOffer'><div class='dha-has-valid-err' id='robo_err_counterOffer'></div><div class='inline-box'><input type='text' class='dha-input-count-offer' name='counterPrice'  placeholder='Counter offer' id='robo_counter_price'>"+selectCurrency+"<button type='button' id='send_counter_price_btn' onClick='formTestsubmit()' class='dha-button-offer-submit'>Submit</button></div> </div>"];
    ...
    chatMsgs[2][1] = ["<div id='robo_buyer_data_formdiv'><form class='msg-width'  id='robo_add_buyer_offer_form'><div class='dha-has-valid-err' id='RN_err_negoForm'></div><input type='text' id='buyer_name' class='dha-input-count-offer' name='buyer_name'  placeholder='Name'><input type='email' id='buyer_email' class='dha-input-count-offer' name='buyer_email'  placeholder='Email'><input type='text' id='buyer_phone' class='dha-input-count-offer' name='buyer_phone'  placeholder='Phone'><input type='text' id='buyer_zip' maxlength='6' class='dha-input-count-offer' name='buyer_zip'  placeholder='Zip Code'><input type='text' id='buyer_original_quantity' class='dha-input-count-offer' name='buyer_original_quantity'  placeholder='Quantity'><button type='button' id='robo_add_buyer_offer_btn' onClick='submitBuyerOfferForm()'  class='dha-find-seller-button'>Submit</button></form></div>"];
    chatMsgs[3] = [];
    chatMsgs[3][0] = ["Thanks. Before we process this offer, we would like to know following:"];
    chatMsgs[3][1] = "<div class='msg-width' id='modal_time_pass_1_formdiv'><legend>In the meantime, please answer the following questions so that we may reserve your deal. <br/><br/> How would you like us to reserve your deal?</legend><form id='pre_form_1' onsubmit='pre_form_1onsubmit()' ><select id='payment_amount_agreed' class='dha-input-count-offer'><option value='25'>Charge me a deposit of $25 and reserve my deal</option> <option value='0'>Let the seller send me an invoice.</option></select><legend>How do you feel about the negotiation process? Any comments or suggestions for improvement?</legend><input type='text' id='PredCommentsNego' class='dha-input-count-offer' name='PredCommentsNego'  placeholder='Comments'> <button type='button' id='robo_preference_btn1' onClick='pre_form_1onsubmit()' class='dha-find-seller-button'>Submit</button></form></div>"
    chatMsgs[3][2] = "<div id='modal_trade'>Do you have a trade-in vehicle before we can match your request? <div class='robo-custom-control custom-radio'><input id='trade_yes' type='radio' class='inline-radio' name='trading_option_selector' value='yes'><label for='trade_yes'><span>&nbsp;</span>Yes</label>&nbsp;&nbsp;&nbsp;<input id='trade_no' type='radio' class='inline-radio' name='trading_option_selector' value='no'><label for='trade_no'><span>&nbsp;</span>No</label></div><div align='right'><button type='button' class='dha-find-seller-button' id='trading_yes_no_btn' onclick='find_trading(event)'>Submit</button><span id='seller_error' style='color:red;'></span></div></div>"
    chatMsgs[3][3] = "<div id='modal_trading_price_range'>Would you like to finance for the purchase OR pay the price (Cash/Credit Card/Check)? <div class='robo-custom-control custom-radio'><input id='trading_price_range_finance' type='radio' class='inline-radio' name='trading_price_type_Selector' value='finance'><label for='trading_price_range_finance'><span>&nbsp;</span>Borrow</label>&nbsp;&nbsp;&nbsp;<input id='trading_price_range_pay_cash' type='radio' class='inline-radio' name='trading_price_type_Selector' value='pay_cash'><label for='trading_price_range_pay_cash'><span>&nbsp;</span>Pay</label></div><div align='right'><button type='button' class='dha-find-seller-button' id='trading_price_range_btn' onclick='find_tradin_price_range(event)'>Submit</button><span id='seller_error' style='color:red;'></span></div></div>"
    chatMsgs[4] = [];
    ...
    chatMsgs[5] = [];
    chatMsgs[5][0] = ["This is your last chance. Please enter your highest and final offer.<br/><div class='msg-width' id='modal_highest_offer'><div class='dha-has-valid-err' id='robo_err_highestOffer'></div><div class='inline-box'><input type='text' class='dha-input-count-offer' name='highest_offer_robo'  placeholder='Enter your final/highest offer' id='highest_offer_robo'> <button type='button' id='send_highest_offer' onClick='updateMyOffer()' class='dha-button-offer-submit'>Submit</button></div> </div>"];
    chatMsgs[5][2] = "<div id='modal_wish_for_highest_offer'>Would you like to update your offer?<div class='robo-custom-control custom-radio'><input id='procees_for_height_offer_yes' type='radio' class='inline-radio' name='finalPriceProgressSelector' value='Yes'><label for='procees_for_height_offer_yes'><span>&nbsp;</span>Yes</label>&nbsp;&nbsp;&nbsp;<input id='procees_for_height_offer_no' type='radio' class='inline-radio' name='finalPriceProgressSelector' value='No'><label for='procees_for_height_offer_no'><span>&nbsp;</span>No</label></div><div align='right'><button type='button' class='dha-find-seller-button' id='process_for_last_offer_valD' onclick='processForHeighestOffer(event)'>Submit</button><span id='final_ofer_error' style='color:red;'></span></div></div>";
}

function setCounterOfferData(counterOffer){
    if(document.getElementById('counterOffer')){
        var counterOfferData = "<div class='inline-box'><input style='width:75%' type='text' disabled='disabled' class='dha-input-count-offer'  placeholder='Please Enter Counter Offer' value='"+counterOffer+"'></div>"+selectCurrency;
...
function setPrefSelectData(){
    if(document.getElementById('modal_time_pass_1_formdiv')){
        ...
        var counterSelectData = "<div class='msg-width'><hr/><legend>We are checking with seller. Meanwhile can we know your preferences for following things: <br/><br/> Choose your option to secure the deal</legend><div ><select class='dha-input-count-offer'><option>"+prefSelected+"</option></select><legend>Give us your comment on how you feel about this negotiation process and how can we improve it?</legend><input type='text' disabled='disabled' class='dha-input-count-offer'  placeholder='Comments' value='"+txtVal+"'></div></div>";
...
function _send (data) {
    ...
    if (sender === "me") setTimeout(_agentReply, 1500);
    // ref to new DOM .msg
}
...
function _autoSend (data) {
    ...
    //ROBO_PASS_COLOR to buttons and texts
    ...
    // $("#payment_amount_agreed > option").hover(function(){
    //     $(this).css("background-color", ROBO_PASS_COLOR);
    // });
    ...
    if (count_progress < 3) {
        if (count_progress === 1) {
            ...
            if (counterPrice !== undefined) {
                ...
                let theEnvelope = {
                    msg: "Thanks for your Counteroffer of <b> - " + roboCurrency +" "+ counterPrice + " </b>",
                    sender: false
                };
                _autoSend(theEnvelope);
                ...
                xmlhttp.onreadystatechange = function () {
                    if (this.readyState === 4 && this.status === 200) {
                        ...
                        if (instantDealCheck.deal_exist === 'no') {
                            ...
                            hasDeal = true;
                            let theEnvelope = {
                                msg: "It looks like we have a special deal from this seller. We’ll need your details in order to match your offer with the seller. Please enter them below:",
                                sender: false
                            };
                            _autoSend(theEnvelope);
                            setTimeout(function () {
...
function updateMyOffer(){
    ...
    let theEnvelope = {
        msg: " " + roboCurrency +" "+ robo_counter_offer_price + " ",
        sender: false
    };
...
function submitBuyerOfferForm(e) {
    ...
    if(buyer_offer_form.buyer_zip.value.toString() =='' || buyer_offer_form.buyer_zip.value.toString().length > 6 || buyer_offer_form.buyer_zip.value.toString().length < 5){
        err=true;
        errMsg +="<span>Enter Valid Zip Code</span>";
    }
    ...
    if(isAgreeCalled == 1){
        ...
        var alertmssg5 = 'Thanks for your information. <br/> Please answer the following questions.';
        theEnvelope = {
            msg: alertmssg5,
            sender: false ,
...
function displayModal(querySelector) {
    ...
    if(querySelector == 'modal_find_seller_yes'){
        ...
        //if(!document.getElementById('modal_wish_for_highest_offer')){
            ...
            var highestOffer = "<div id='modal_wish_for_highest_offer'>Would you like to update your offer? <div class='robo-custom-control custom-radio'><input id='procees_for_height_offer_yes"+highCounter+"' type='radio' class='inline-radio' name='finalPriceProgressSelector' value='Yes'><label for='procees_for_height_offer_yes"+highCounter+"'><span>&nbsp;</span>Yes</label>&nbsp;&nbsp;&nbsp;<input id='procees_for_height_offer_no"+highCounter+"' type='radio' class='inline-radio' name='finalPriceProgressSelector' value='No'><label for='procees_for_height_offer_no"+highCounter+"'><span>&nbsp;</span>No</label></div><div align='right'><button type='button' class='dha-find-seller-button' id='process_for_last_offer_valD' onclick='processForHeighestOffer(event)'>Submit</button><span id='final_ofer_error' style='color:red;'></span></div></div>";
            // textClass = 'text-success';
            let theEnvelope = {
                msg: highestOffer,
                sender: false
...
function callAgree() {
    // textClass = 'text-danger';
    var alertmssg = "Do you agree to receive emails and text messages for this request? <div class='robo-custom-control custom-radio'><input id='agree_terms_yes' type='radio' class='inline-radio' name='agree_terms_select' value='yes'><label for='agree_terms_yes'><span>&nbsp;</span>Yes</label>&nbsp;&nbsp;&nbsp;<input id='agree_terms_no' type='radio' class='inline-radio' name='agree_terms_select' value='no'><label for='agree_terms_no'><span>&nbsp;</span>No</label></div><div align='right'><button type='button' class='dha-find-seller-button' id='check_agree_btn' onclick='onClickAgree(event)'>Submit</button><span id='agree_error' style='color:red;'></span></div>"
    theEnvelope = {
        msg: alertmssg,
        sender: false ,
    ...
    xmlHttp.onreadystatechange = function () {
        ...
        if (hasDeal === null) {
            ...
            if (final_offer_submitted < 1) {
                ...
                $mesg = 'Thanks for your new offer! We hope that your new offer will match with the seller. Please wait while we check.';
            }
            if($mesg !== '') {
                theEnvelope = {
                    msg: $mesg,
            ...
            if (data2.message !== undefined) {
                if (data2.message !== undefined) {
                    if (data2.message === 'failed') {
                        ...
                        final_message = {
                            msg: '<span>We’re sorry, but we couldn’t find a match for your offer. Would you like to update your offer?</span>',
                            sender: false ,
                            elementId : 'robo-xcdgbgrt'
                        };
                        if (preferences !== undefined) {
                        ...
                        if (final_offer_submitted < 1) {
                            ...
                            // textClass = 'text-danger';
                            final_message = {
                                msg: '<span>We\'re sorry, but we couldn\'t find a match for your offer. You can update your offer and try again.</span>',
                                sender: false ,
        ...
        xmlhttp.onreadystatechange = function () {
            ...
            if (this.readyState === 4 && this.status === 200) {
                ...
                if (data.closed_deal === undefined) {
                    ...
                    final_message = {
                        msg: '<span style="color: #4caf50">Congratulations! The seller has agreed to sell product: <span style="color: red">'+ROBO_UPC_CODE+'</span> at <b><span style="color: red">'+roboCurrency+'</span> <span style="color: red">'+closedDealFinalAmount+'</span></b>.<br/> To connect with our seller, please contact them through their email: <b><span style="color: red">' +closedDealEmail+ '</span></b><br/> For record-keeping purposes, your deal ID is #<b><span style="color: red">' + buyer_offer_request_id + '</span></b></span>',
                        sender: false ,
                    ...
                    if (preferences !== undefined) {
                        ...
                        if(parseInt(data.closed_deal[0].total_discounted_amount) > 0) {
                            ...
                            var discountMssg = '<span style="color: 4caf50"><br/> Although the agreed price is <b><span style="color: red">'+roboCurrency+'</span> <span style="color: red">'+closedDealFinalAmount+'</span></b>, we have applied your discount of <b><span style="color: red">'+roboCurrency+' ' +data.closed_deal[0].total_discounted_amount+'</span></b>. Hence, your cost will be <b><span style="color: red">'+roboCurrency+' '+actualAmount+'</span></b>. Please note that <b>sales tax and shipping</b> are not included in this price.</span>';
                            discountMssg = {
                                msg: discountMssg,
                                sender: false ,
                        ...
                        if (agreedAmount !== 0) {
                            ...
                            var depositMessage = {
                                msg: 'To secure your deal, please pay your deposit of <b> '+ roboCurrency + " " + agreedAmount + '</b> to the seller.',
                                sender: false ,
                            ...
                            closeChat = 1;
                            let theEnvelop = {
                                msg: "Great! We\'ll let the seller know that you would like an invoice.",
                                sender: false
                            };
                            _autoSend(theEnvelop);
                            var invoiceOption = "<b>Please note, the deal may not still be there through this option.</b><br/><br/><input type='checkbox' id='invoice2' name='invoice2' value='agree'><label for='invoice2'> I understand</label><br/><br/><button type='button' id='let_seller_send_invoice' onClick='closeChatBotRobo()' class='dha-find-seller-button'>Submit</button>";
                            setTimeout(function() {
                                let theEnvelop = {
                                    msg: invoiceOption,
                                    sender: false
...
function pre_form_1onsubmit(e) {
    ...
    let theEnvelop = {
        msg: 'Thanks! Please wait while the matching service finishes.',
        sender: false
    };
    _autoSend(theEnvelop);
    setTimeout(function() {
...
function get_discounts() {
    // textClass = 'text-danger';
    var alertmssg = "Great! Let’s check if there are any special discounts or rebates you qualify for.  Any other discounts will automatically be applied to your deal."
    theEnvelope = {
        msg: alertmssg,
        sender: false ,
            ...
            if (data.success !== undefined && data.success === false) {
                ...
                if (Array.isArray(data.data) && data.data.length) {
                    ...
                    if(specificAvailable === 1) {
                        // textClass = 'text-danger';
                        var slectDisT = "Please select \"Yes\" ONLY IF you qualify for the discount: "
                        theEnvelope = {
                            msg: slectDisT,
                            sender: false ,
                        ...
                        if(couponCodes.length > 0 && couponSection == 0) {
                            ...
                            // textClass = 'text-success';
                            let theEnvelope2 = {
                                msg: 'Thanks! Please wait while our system finds a match for your offer.',
                                sender: false
                            };
...
//Ask for coupon code 26-Jul-2020 changed
function askForCoupon() {
    var askCouponMssg = "Please enter your coupon code here if you have one. <div id='couponcodehere'><div class='dha-has-valid-err' id='robo_err_co'></div><div class='inline-box'><input type='text' class='dha-input-count-offer' name='coupon_code'  placeholder='Coupon Code' maxlength='15' id='robo_coupon_code'> <button type='button' id='robo-skipcouponcode' onClick='couponNextStep()' class='dha-find-seller-button' style='float: left; width: 40%;'>Skip</button> <button type='button' id='robo-sendcouponcode' onClick='addCouponCode()' class='dha-find-seller-button' style='float: right; width: 40%;'>Submit</button></div> </div>";
    let theEnvelope = {
        msg: askCouponMssg,
        sender: false
    };
...
//Skip coupon code 26-Jul-2020 changed
function couponNextStep() {
    let theEnvelop = {
        msg: "No, I don't have <b>coupon code</b>.",
        sender: false
    };
    _customerReply(theEnvelop);
    addCouponCode();
...
//Add coupon code 26-Jul-2020 changed
function addCouponCode() {
    if(document.getElementById('robo_coupon_code')) {
        ...
        document.getElementById("robo-skipcouponcode").style.display = "none";
        if(userCouponCode != '') {
            let theEnvelop = {
                msg: "Yes, I have a <b>coupon code</b>.",
                sender: false
            ...
            if(couponCodes.includes(userCouponCode.toLowerCase())){
                var couponaddmssg = "Thanks! We will apply your coupon if your offer is matched with the seller.";
                appliedCouponCode = userCouponCode.toLowerCase();
            } else {
                var couponaddmssg = "We're sorry, but your coupon code was invalid. Please try again later.";
            }
            document.getElementById("robo-sendcouponcode").style.display = "none";
            document.getElementById("robo-skipcouponcode").style.display = "none";
            // textClass = 'text-danger';
