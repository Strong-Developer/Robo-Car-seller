...
class PatientController extends Controller
{
    ...
    public function EditPatient(Request $request,$id)
    {
        ...
        $eyeVistDa = EyeVisitData::where('patient_id', $id)->select('eye_visit_data.created_at','eye_visit_data.unique_id','eye_visit_data.user_id');
        if($user->role_type == 3) {
            $appoint->where('doctro_name', '=', $user->id);
            $eyeVistDa->where('user_id', '=', $user->id);
        }
        $allCustomEyeData = $eyeVistDa->get();
    	...
    	return view('admin.editPatient', ['patientData' => $patientData, 'appointments' => $appointments, 'privacy'=>$privacy,'user'=>$user,'wacomQuestions'=>$wacomQuestions,'allCustomEyeData'=>$allCustomEyeData,'allUsers'=>$allUsers]);
    }
    ...
    public function eyevisit(Request $request,$patid,$uniqueid=null) {
        $user = auth()->user();
        ...
        $eyeDataPat=EyeVisitData::where('patient_id', $patid)->select('eye_visit_data.eye_visit')->where('unique_id', '=', $uniqueid)->first();
        if($request->method() == 'POST') {
            $eyeData['eye_visit'] = json_encode(Input::get('eye_visit'));
            if(empty($uniqueid)) {
                $eyeData['user_id'] = $user->id;
                $eyeData['unique_id'] = uniqid();
                $uniqueid = $eyeData['unique_id'];
            }
            $mage = EyeVisitData::firstOrNew(['patient_id'=>$patid,'unique_id'=>$uniqueid], $eyeData);
            if($mage->exists){
                EyeVisitData::where('id', $mage->id)->update($eyeData);
            } else {
                $mage->save();
            }
            return redirect('/admin/eyevisit/'.$patid.'/'.$uniqueid)->with('success',"I dati sono stati aggiornati correttamente.");
        }
        return view('admin.eyeVisit', ['eyeVisitTabs'=>$eyeVisitTabs,'eyeVisitInputTabs'=>$eyeVisitInputTabs,'eyeDataPat'=>$eyeDataPat,'patid'=>$patid,'patientData'=>$patientData]);
    }

    public function eyevisitAppointment(Request $request,$patid, $appid) {
        $eyeVisitTabs = EyeVisitTabs::where(['status'=>1])->get();
        $inputTabs = InputTabs::where(['status'=>1])->get();
        $patientData = Patient::where(['id'=>$patid])->select('patients.surname','patients.name')->first();
        $eyeVisitInputTabs = [];
        ...
        if($request->method() == 'POST') {
            ...
            return redirect('/admin/eyevisit-appuntamento/'.$patid.'/'.$appid)->with('success',"I dati sono stati aggiornati correttamente.");
        }
        return view('admin.eyeVisitAppointment', ['appointmentData' => $appointmentData,'eyeVisitTabs'=>$eyeVisitTabs,'eyeVisitInputTabs'=>$eyeVisitInputTabs,'eyeDataPat'=>$eyeDataPat,'patid'=>$patid,'patientData'=>$patientData]);
    }
    ...
    public function managepatient() {
        ...
        $patientFirst = [];
        $appointMedicines = DB::table('appointment_medicine')
                        ->select('appointment_medicine.medicine','appointment_medicine.appointment_id')
                        ->where(DB::raw("DATE_FORMAT(appointment_medicine.created_at, '%Y-%m-%d')"), '=', date('Y-m-d',time()))->get();
        $medData = [];
        foreach($appointMedicines as $med) {
            $medData[$med->appointment_id][] = $med->medicine;
        }
        $todaysDoc =[];
        $patInDatabase = [];
        ...
        return view('admin.patientManagement', ['patients' => $patientsOfTheDay,'existingPat'=>$existingPat,'user'=>$user,'patientFirst'=>$patientFirst,'examination'=>$examination,'doctorData'=>$doctorData, 'medData'=>$medData]);
    }
    ...
    public function getpatient(Request $request, $appid) {
        ...
        $medicines = AppointmentMedicine::where(['appointment_id'=>$appid])
                    ->join('users', 'appointment_medicine.user_id', '=', 'users.id')
                    ->select('appointment_medicine.*','users.name as userName') 
                    ->get();  
    ...
    public function savemedicine(Request $request) {
        if(!empty(Input::get('medicine'))) {
            ...
            $appointmentMedicine->medicine =Input::get('medicine');
            $appointmentMedicine->appointment_id =Input::get('app_id');
            $appointmentMedicine->user_id =$user->id;
            if($appointmentMedicine->save()) {
                setlocale(LC_TIME, 'it_IT');
                echo $user->name.' ha scelto '.Input::get('medicine').' alle '.strftime('%I:%M', time());  
            }
    ...
    public function updatedailypat()
    {
        ...
        if(isset($existingPat->u_id)) {
            ...
            if(in_array($user->id,$userIdArray)) {
                ...
                $patientsOfTheDay = DB::table('appointement_booking')
                                    ->join('patients', 'appointement_booking.patient_id', '=', 'patients.id')                     
                                    ->select('appointement_booking.id as appointid','appointement_booking.starteTime','appointement_booking.endtime','appointement_booking.doctro_name as docId', 'patients.*')  ->where('patients.type', '=', '1')                  
                                    ->where(DB::raw("DATE_FORMAT(appointement_booking.start_date, '%Y-%m-%d')"), '=', date('Y-m-d',time()))->get()->keyBy('id');
                $appointMedicines = DB::table('appointment_medicine')
                                ->select('appointment_medicine.medicine','appointment_medicine.appointment_id')
                                ->where(DB::raw("DATE_FORMAT(appointment_medicine.created_at, '%Y-%m-%d')"), '=', date('Y-m-d',time()))->get();
                ...
                return view('admin.dailyPat', ['patients' => $patientsOfTheDay,'existingPat'=>$existingPat,'user'=>$user,'patientFirst'=>$patientFirst,'examination'=>$examination,'doctorData'=>$doctorData,'medData'=>$medData]);
            }
        }
        exit;
    }
